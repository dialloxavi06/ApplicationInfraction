...
class DelitDAO
{
	...
	{
		$this->bd->execSQL(
			"INSERT INTO delit (nature, montant)
	...
	{
		$this->bd->execSQL(
			"DELETE FROM delit WHERE id_delit = :id_delit",
			[':id_delit' => $id_delit]
		);
	}
	...
	{
		$this->bd->execSQL(
			"UPDATE delit SET nature=:nature, montant=:montant WHERE id_delit=:id",
			[':nature' => $Delit->getNature(), ':montant' => $Delit->getMontant(), ':id' => $Delit->getId_delit()]
		);
	}
	...
	private function loadQuery(array $result): array
	{
		...
		foreach ($result as $row) {
			$Delit = new Delit();
			$Delit->setId_delit($row['id_delit']);
	...
	{
		$unDelit = new Delit();
		$lesDeilts = $this->loadQuery($this->bd->execSQL($this->select . " WHERE id_delit=:id_delit", [':id_delit' => $id_delit]));
		if (count($lesDeilts) > 0) {
			$unDelit = $lesDeilts[0];
		}
		return $unDelit;
	}
	...
	function getNonDelit ( $numInfraction) {
		return	($this->loadQuery($this->bd->execSQL($this->select 
		." WHERE id_delit NOT IN (SELECT id_delit FROM COMPREND WHERE id_inf=:num)", [':num'=>$numInfraction]) ));
	}
